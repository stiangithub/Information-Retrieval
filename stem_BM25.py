...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 20 12:40:49 2018
...
#     files= glob(os.path.join('stem_corpus','*.txt'))
#     doc_list = []
#     rel_list = []
...
def write_scores_to_file(scores,query_id):
    file1=open("results/stem_bm25/stem_bm25_query"+str(query_id)+".txt",'w')
    rank = 0
    for each in scores:
        rank +=1
        if rank <=100:
            file1.write((str(query_id)+' '+'Q0'+' '+each[0]+' '+str(rank)+' '+str(each[1])+' '+'BM25'+'\n'))
    file1.close()


def save_queries(queries):
    file_name = "stem_queries.txt"
...
def main():
    ...
    for query in queries:
        query_id += 1
        bm25_scores=bm25(query.split(),query_id,avdl)
