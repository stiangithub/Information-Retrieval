...
def write_scores_to_file(tf_idf_scores, query_id):
    file1 = open("results/tf_idf_stopped/tf_idf_query" + str(query_id) + ".txt", 'w')
    rank = 0
...
def main():
    uni_index = generate_stopped_unigram()  # Unigram Task2 and 3
    ...
    for query in queries:
        query_id += 1
        tf_idf_scores = generate_tf_idf_scores(query, index)
